...
installCmd
	...
	.action(async ({eas, platform}: {eas?: boolean; platform: string}) => {
		if (!eas) {
			console.error('Only --eas installs are currently supported.');
			process.exit(1);
		}

		if (platform === 'android') {
			console.error('Android install is not yet implemented.');
			process.exit(1);
		}
